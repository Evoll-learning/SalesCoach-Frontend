...
const SimulatorPage = ({ user }) => {
  ...
  // Update sectors when data is loaded
  React.useEffect(() => {
    if (sectorsData) {
  ...
  const handleFileUpload = async (event) => {
    ...
    // TODO: Implement document upload with tRPC
    // For now, just store file names locally
    try {
      setLoading(true);
      const fileNames = validFiles.map(f => ({ name: f.name, size: f.size }));
      setFormData(prev => ({
        ...prev,
        supportDocuments: [...prev.support_documents, ...fileNames]
      }));

      toast.success(`${validFiles.length} archivo(s) añadido(s) (pendiente de subir)`);
      toast.info('La subida de documentos se implementará próximamente');
  ...
  const canStart = () => {
    return formData.simulationType && 
           ...
           formData.sectorCode &&  // NEW: Sector validation
  ...
  // Waiting state - after Tavus window opens
  if (step === 'waiting') {
    return (
      <div className="min-h-screen simulator-bg">
        <nav className="glass border-b border-white/20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center py-4">
              <div className="brand-logo">SalesCoach AI</div>
            </div>
          </div>
        </nav>
          ...
          <Card className="card text-center">
            <CardHeader>
              ...
              <CardDescription className="text-lg">
                Tu simulación se abrió en una nueva ventana
              </CardDescription>
            </CardHeader>
            ...
            <CardContent className="space-y-6">
              <div className="bg-blue-50 p-6 rounded-lg">
                ...
                <ol className="text-left space-y-2 max-w-md mx-auto">
                  <li className="flex items-start">
                    ...
                    <span>Completa tu conversación en la ventana de Tavus</span>
                  </li>
                  <li className="flex items-start">
                    <span className="font-bold mr-2">2.</span>
                    <span>Cuando termines, cierra esa ventana</span>
                  </li>
                  <li className="flex items-start">
                    <span className="font-bold mr-2">3.</span>
                    <span>Vuelve aquí y haz click en "Ver mi Feedback"</span>
                  </li>
                </ol>
              </div>
              ...
              <div className="space-y-3">
                <Button
                  size="lg"
                  onClick={() => navigate(`/feedback?id=${conversationData?.conversation_id}`)}
                  className="btn-primary px-8 py-4 text-lg"
                >
                ...
                <p className="text-sm text-slate-600">
                  El feedback estará listo en 1-2 minutos después de terminar
                </p>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <Card className="card">
            <CardHeader className="text-center">
              ...
              <CardDescription>
                {formData.simulationType} • {formData.objective} • Lead {formData.leadTemperature}
              </CardDescription>
            </CardHeader>
    ...
    <div className="min-h-screen simulator-bg">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-8">
                  ...
                  return (
                    <div 
                      ...
                      className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                        formData.simulationType === type.id 
                          ? 'border-blue-500 bg-blue-50' 
                          : 'border-slate-200 hover:border-blue-300'
                      }`}
                      onClick={() => setFormData({...formData, simulationType: type.id})}
                    >
          ...
          {/* Sales Type: B2B or B2C */}
          <Card className="card">
            <CardHeader>
              <CardTitle>Tipo de Venta</CardTitle>
              <CardDescription>
                ¿Vendes a empresas (B2B) o a consumidores finales (B2C)?
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                <div 
                  className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                    formData.salesType === 'b2b' 
                      ? 'border-blue-500 bg-blue-50' 
                      : 'border-slate-200 hover:border-blue-300'
                  }`}
                  onClick={() => setFormData({...formData, salesType: 'b2b'})}
                >
                  <div className="text-center">
                    <Badge className="mb-3 bg-indigo-100 text-indigo-800">B2B</Badge>
                    <p className="font-semibold mb-1">Venta a Empresas</p>
                    <p className="text-sm text-slate-600">Ciclos largos, decisores múltiples</p>
                  </div>
                </div>
                <div 
                  className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                    formData.salesType === 'b2c' 
                      ? 'border-blue-500 bg-blue-50' 
                      : 'border-slate-200 hover:border-blue-300'
                  }`}
                  onClick={() => setFormData({...formData, salesType: 'b2c'})}
                >
                  <div className="text-center">
                    <Badge className="mb-3 bg-green-100 text-green-800">B2C</Badge>
                    <p className="font-semibold mb-1">Venta a Consumidores</p>
                    <p className="text-sm text-slate-600">Decisión rápida, emocional</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="card">
            <CardHeader>
              <CardTitle>Sector o Industria</CardTitle>
              <CardDescription>
                Selecciona tu sector para una simulación más personalizada. Si no encuentras el tuyo, elige "Otro Sector".
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-3 gap-4">
                {sectors.map((sector) => (
                  <div 
                    ...
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                      formData.sectorCode === sector.sector_code
                        ? 'border-blue-500 bg-blue-50' 
                        : 'border-slate-200 hover:border-blue-300'
                    }`}
                    onClick={() => setFormData({...formData, sectorCode: sector.sector_code})}
                  >
                    <div className="text-center">
                      <div className="text-3xl mb-2">{sector.icon}</div>
                      <p className="font-semibold mb-1">{sector.sector_name}</p>
              ...
              {/* Custom Sector Input - Show only when OTHER is selected */}
              {formData.sectorCode === 'OTHER' && (
                <div className="mt-6 p-4 bg-blue-50 border-2 border-blue-200 rounded-lg">
                  <Label htmlFor="custom_sector" className="text-sm font-semibold text-blue-900 mb-2 block">
                    ¿Cuál es tu sector específico?
                  </Label>
                  <Input
                    id="custom_sector"
                    placeholder="Ej: Inmobiliarias, Educación, Abogados, Psicología, Coaching..."
                    value={formData.customSector}
                    onChange={(e) => setFormData({...formData, customSector: e.target.value})}
                    className="bg-white"
                  />
                  <p className="text-xs text-slate-600 mt-2">
                    Esto nos ayuda a personalizar mejor tu experiencia de simulación
                  </p>
                </div>
              )}
          ...
          <Card className="card">
            <CardHeader>
              <CardTitle>¿Qué Vendes?</CardTitle>
              <CardDescription>
                ¿Tu oferta es un producto tangible o un servicio?
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                <div 
                  className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                    formData.productService === 'producto' 
                      ? 'border-blue-500 bg-blue-50' 
                      : 'border-slate-200 hover:border-blue-300'
                  }`}
                  onClick={() => setFormData({...formData, productService: 'producto'})}
                >
                  <div className="text-center">
                    <Badge className="mb-3 bg-purple-100 text-purple-800">Producto</Badge>
                    <p className="font-semibold mb-1">Producto Tangible</p>
                    <p className="text-sm text-slate-600">Algo físico que se entrega</p>
                  </div>
                </div>
                <div 
                  className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                    formData.productService === 'servicio' 
                      ? 'border-blue-500 bg-blue-50' 
                      : 'border-slate-200 hover:border-blue-300'
                  }`}
                  onClick={() => setFormData({...formData, productService: 'servicio'})}
                >
                  <div className="text-center">
                    <Badge className="mb-3 bg-cyan-100 text-cyan-800">Servicio</Badge>
                    <p className="font-semibold mb-1">Servicio Intangible</p>
                    <p className="text-sm text-slate-600">Consultoría, software, etc.</p>
                  </div>
                </div>
              </div>
            </CardContent>
          ...
          <Card className="card">
            ...
            <CardContent>
              ...
              >
                {objectives.map((obj) => (
                  <div key={obj.id} className="flex items-center space-x-3 p-3 border rounded-lg hover:border-blue-300 transition-colors">
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {leadTemperatures.map((temp) => (
                  <div 
                    key={temp.id}
                    className={`p-4 border-2 rounded-lg cursor-pointer transition-all ${
                      formData.leadTemperature === temp.id 
                        ? 'border-blue-500 bg-blue-50' 
                        : 'border-slate-200 hover:border-blue-300'
                    }`}
                    onClick={() => setFormData({...formData, leadTemperature: temp.id})}
                  >
                    <div className="text-center">
          ...
          <Card className="card">
            <CardHeader>
              ...
              <CardDescription>
                Elige el género del avatar con quien practicarás
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
                {avatarOptions.map((avatar) => (
                  <div 
                    ...
                    className={`p-6 border-2 rounded-lg cursor-pointer transition-all ${
                      formData.avatarGender === avatar.id 
                        ? 'border-blue-500 bg-blue-50' 
                        : 'border-slate-200 hover:border-blue-300'
                    }`}
                    onClick={() => setFormData({...formData, avatarGender: avatar.id})}
                  >
                    <div className="text-center">
                      <div className="text-4xl mb-3">{avatar.icon}</div>
                      <h3 className="font-semibold mb-1">{avatar.name}</h3>
                      <p className="text-sm text-slate-600">{avatar.description}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <Card className="card">
            <CardHeader>
              <CardTitle className="flex items-center">
                ...
                Idioma de Simulación
              </CardTitle>
              <CardDescription>
                Selecciona el idioma para la conversación
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid md:grid-cols-2 gap-4">
          ...
          <Card className="card">
            <CardHeader>
              ...
              <CardDescription>
                Sube documentos para personalizar la simulación (PDF, DOC, TXT - máx. 10MB cada uno)
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="border-2 border-dashed border-slate-300 rounded-xl p-8 text-center hover:border-blue-400 transition-colors">
                ...
                <label htmlFor="file-upload" className="cursor-pointer">
                  ...
                  <p className="text-sm text-slate-500 mb-4">
                    Sube brochures, presentaciones, catálogos, scripts, etc.
                  </p>
                  <div className="text-xs text-slate-400">
                    Formatos: PDF, TXT, DOC, DOCX | Máximo: 10MB por archivo
                  </div>
                </label>
              </div>
              ...
              {formData.supportDocuments.length > 0 && (
                <div className="space-y-2">
                  <h4 className="font-semibold text-slate-700 mb-3 flex items-center">
                    ...
                    Archivos subidos ({formData.supportDocuments.length}):
                  </h4>
                  {formData.supportDocuments.map((file, index) => (
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            {file.size ? (file.size / 1024 / 1024).toFixed(2) : '0'} MB • {file.content_type}
                          </p>
                        </div>
                      </div>
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
                <h4 className="font-semibold text-amber-800 mb-2">💡 Cómo usar los documentos:</h4>
                <ul className="text-sm text-amber-700 space-y-1">
                  <li>• <strong>Brochures:</strong> El avatar conocerá tus productos/servicios</li>
                  <li>• <strong>Casos de uso:</strong> Podrá mencionar historias de éxito reales</li>
                  <li>• <strong>Precios:</strong> Manejará objeciones con datos específicos</li>
                  <li>• <strong>Scripts:</strong> Seguirá tu metodología de ventas</li>
                </ul>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="text-center space-y-4">
            ...
            <div>
                ...
                onClick={async () => {
                  try {
                    ...
                    if (response.data.conversation_url) {
                      window.open(response.data.conversation_url, '_blank', 'noopener,noreferrer');
                      toast.success('Simulación abierta en nueva ventana');
                    }
                  } catch (error) {
              ...
              >
                ...
                Probar directo (sin permisos)
              </Button>
            </div>
          </div>
        </div>
        
        {/* Footer with Powered by EvollAI */}
        <div className="text-center mt-8 pb-4">
          <p className="text-slate-400 text-xs">
            Powered by <span className="font-semibold text-blue-500">EvollAI</span>
          </p>
        </div>
      </div>
    </div>
